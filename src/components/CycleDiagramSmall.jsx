...
const CycleDiagramSmall = ({ currentRealm, playableRealms = [], isReversed }) => {
    ...
    const { width, isPortrait, isMicro } = dimensions;
    ...
    const mBase = isPortrait 
        ? (isMicro ? Math.max(90, 110 - (480 - width) * 0.1) : 130) 
        : 180;

    const mScale = isPortrait ? (isMicro ? 1.4 : 1.8) : 3.0;
    const fontSize = isPortrait ? (isMicro ? 16 : 20) : 28;
    ...
    return (
        <div ref={containerRef} className="tactical-cycle-wheel" style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', pointerEvents: 'none', zIndex: 2 }}>
            <svg viewBox="0 0 800 600" style={{ 
                ...
                {items.map((item, i) => {
                    const pos = getPos(i);
                    const nextPos = getPos((i + (isReversed ? 5 : 1)) % 6);
                    ...
                    return (
                        <g key={item.k}>
                            <line
                                ...
                                stroke={isCurrent ? "rgba(255,255,255,0.8)" : "rgba(255,255,255,0.12)"}
