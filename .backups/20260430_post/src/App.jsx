...
const App = () => {
    ...
    }, [gs?.fieldCard, visualFieldCard, isAnimating]);
    ...
    useEffect(() => {
        if (gs && gs.fieldCard && gs.fieldCard.id !== prevFieldCardId.current) {
            ...
            if (c.wasPlanet || c.wasRuins || c.wasFountain) {
                console.log("[ANIM] WILD detected. Starting 3s freeze...");
                setIsAnimating(true);
                setVisualFieldCard({
                    ...c,
                    realm: c.wasPlanet ? 'PLANET' : (c.wasRuins ? 'RUINS' : 'FOUNTAIN'),
                ...
                morphTimeoutRef.current = setTimeout(() => {
                    console.log("[ANIM] 1.5s passed. Starting 1.5s cross-fade to:", c.realm);
                    setVisualFieldCard(c);
                    setIsAnimating(false);
                    setIsMorphing(true);
                    // フェードが終わるまで待機
                    setTimeout(() => setIsMorphing(false), 1500);
                        ...
                        <div className="w-full mt-2 px-4 flex flex-col items-center">
                            {(gs?.players[0]?.id === socket?.id || gs?.players[0]?.name === name) && (
                                <div className="flex gap-2 w-full mb-3">
                                    <button className="flex-1 py-4 px-1 bg-black/80 border border-white/20  text-white font-black text-[11px] tracking-[1px] uppercase rounded-sm whitespace-nowrap" disabled={gs?.players?.length >= 5} onClick={() => {
                                        playSE('play', muted);
        if (window.navigator.vibrate) window.navigator.vibrate(12);
        ...
        +{p.basePoints || p.earnedPoints}{p.bonusPoints > 0 ? ` + ${p.bonusPoints}` : ''}
                    ...
                    <>
                        <div className="flex justify-between items-center px-4 py-2 bg-[#05010a]/90 border-b border-accent/20 shrink-0 z-50">
                            ...
                            <div className="w-8 h-8 rounded-full border-2 border-accent flex items-center justify-center text-accent bg-black/80  cursor-pointer shadow-[0_0_10px_rgba(64,224,208,0.4)] transition-all ml-2" onClick={() => setBgAnim(!bgAnim)} title={bgAnim ? "演出をオフにする" : "演出をオンにする"}>
                            ...
                            <div className="w-8 h-8 rounded-full border-2 border-accent flex items-center justify-center text-accent bg-black/80  cursor-pointer shadow-[0_0_10px_rgba(64,224,208,0.4)] transition-all ml-2" onClick={() => setMuted(!muted)}>
                        ...
                        <div className={`hand-container no-scrollbar ${isMyTurn ? 'my-turn-hand-fx' : ''} ${(isAnimating || isMorphing) ? 'opacity-40 grayscale-[50%] pointer-events-none' : ''}`}>
                            {sortedHand.map((card, idx) => {
                                ...
                                const baseMargin = -12;
                                const dynamicMargin = handSize > 7 
                                    ? Math.max(-50, baseMargin - (handSize - 7) * 6) 
                                    : baseMargin;
                                ...
                                return (
                                    <div 
                                        key={card.id || idx} 
                                        className={`card-anchor ${selectedCardId === card.id ? 'selected' : ''} ${!isMyTurn || !isPlayable ? 'not-playable' : 'playable'} ${isMyTurn ? 'is-my-turn' : ''} ${isPlayable ? 'playable-card-pop' : ''}`}
                                        style={{ 
                        ...
                        <div className="w-full px-4 pb-4 shrink-0 flex flex-col gap-2">
                            <div className="flex justify-between items-end px-1">
                                ...
                                {me?.hand.length >= 8 && !(isAnimating || isMorphing) && (
