Cross Realm - Eternal Punk Edition -

スチームパンク、幻想世界、サイバーパンクという3つの異なる世界観（レルム）が交差する、オンライン・タクティカル・カードゲームです。
洗練されたネオンとホログラム、そして完全CSS+SVGで表現された美麗なUIで、ブラウザ上で白熱したリアルタイム対戦を楽しめます。

⚡ クイックスタート

プレイヤー名: 自分の名前を入力（最大10文字）。

ルームID: 対戦相手と同じ合言葉（ID）を入力。

接続開始: ルームに入場し、全員が揃ったらホスト（一番最初に入った人）が「ゲーム開始」を押してミッションスタート。
...
出せるカードがない、または戦略的に出したくない場合は、カードを1枚ドロー（引く）してターンを終了します。
...
| B→C | 🔋 電池 (BATTERY) | 機械 または 古文書 | 【遷移】サイバーへのゲート |
| C | 🤖 機械 (MACHINE) | 機械 または 古文書 | 【維持】サイバーパンク |
| C→A | 📖 古文書 (ARCHIVE) | 歯車 または 氷河期 | 【遷移】スチームパンクへの帰還 |

※ 注意: 「遷移」の役割を持つカード（氷河期・電池・古文書）は、同じカードを連続して重ねることはできません。必ず次の世界観へのゲートとなります。
...
オート・ログシステム: 誰が何を引いたか、何を出したかが左上のサイバーコンソールに流れます（5秒で自動フェードアウト）。

ホログラム・カード: 画像ロードゼロの超軽量設計。世界観に合わせた「銅のテクスチャ」「魔法のパーティクル」「電子スキャンライン」などのフレーム枠がCSSで美しく描画されます。

履歴スタック機能: 場に出た直近のカードが数枚、下部に透けて積み重なる演出を採用。

サウンドエフェクト: 行動ごとの効果音、ターン通知音、攻撃時のアラート音により、高い没入感を提供します。
...
利用について: 本ソースコードの商用利用、および公式のものと誤解を招くような形での再配布・公開を固く禁じます。
